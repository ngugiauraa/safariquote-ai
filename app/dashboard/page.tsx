...
export default function CompanyDashboard() {
  ...
  const [customizationSettings, setCustomizationSettings] =
    useState<CompanyCustomizationSettings>(() => normalizeCompanySettings({}));

  const effectiveSettings = normalizeCompanySettings(customizationSettings, {
    logoUrl,
    contactEmail,
  });
  ...
  }, [organization?.id, organization?.name, user?.emailAddresses]);
    ...
    try {
      ...
      if (result.created && result.company?.contact_email) {
        ...
        if (!welcomeResponse.ok || !welcomeResult.success) {
          throw new Error(welcomeResult.error || 'Company saved, but welcome email failed.');
        }
